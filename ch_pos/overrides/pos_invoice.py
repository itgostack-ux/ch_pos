...
def _hydrate_imported_sales_invoice_defaults(doc):
    ...
    # For imported submitted invoices, ensure each item carrying a serial
    # has a warehouse set. Prefer CH Serial Lifecycle.current_warehouse,
    # then Serial No.warehouse. This ensures stock moves and lifecycle
    # transitions target the correct store/bin when we run post-import hooks.
    try:
        for item in getattr(doc, "items", []) or []:
            # Skip if warehouse already set
            if item.get("warehouse"):
                continue
            serial_field = (item.get("serial_no") or "").strip()
            if not serial_field:
                continue
            # serial may be newline/comma separated — take first token
            first_serial = serial_field.splitlines()[0].split(",")[0].strip()
            if not first_serial:
                continue
            # Prefer CH Serial Lifecycle current_warehouse
            lc_wh = frappe.db.get_value("CH Serial Lifecycle", first_serial, "current_warehouse")
            if lc_wh:
                item.warehouse = lc_wh
                continue
            # Fallback: Serial No.warehouse
            sn_wh = frappe.db.get_value("Serial No", first_serial, "warehouse")
            if sn_wh:
                item.warehouse = sn_wh
    except Exception:
        frappe.log_error(frappe.get_traceback(), f"warehouse inference failed for imported Sales Invoice {getattr(doc, 'name', '(new)')}")
    # If the Data Import created a submitted Sales Invoice (docstatus==1),
    # run best-effort on_submit side-effects that the importer may have
    # skipped: device registration, serial lifecycle transitions, coupon
    # accounting and kiosk token updates. Never raise — import should not
    # fail because a post-process couldn't complete.
    try:
        if getattr(doc, "docstatus", 0) == 1:
            try:
                create_customer_device_records(doc)
            except Exception:
                frappe.log_error(frappe.get_traceback(), f"create_customer_device_records (import) failed for {doc.name}")
            try:
                update_serial_lifecycle(doc)
            except Exception:
                frappe.log_error(frappe.get_traceback(), f"update_serial_lifecycle (import) failed for {doc.name}")
            try:
                update_kiosk_token_status(doc)
            except Exception:
                # non-fatal — kiosk token linkage is optional
                frappe.log_error(frappe.get_traceback(), f"update_kiosk_token_status (import) failed for {doc.name}")
            try:
                increment_coupon_usage(doc)
            except Exception:
                frappe.log_error(frappe.get_traceback(), f"increment_coupon_usage (import) failed for {doc.name}")
            # Ensure Stock Ledger Entries (SLE) and GL entries exist for imported,
            # submitted invoices. Only run if missing to avoid duplicates.
            try:
                # Check for any SLEs for this invoice
                sle_exists = frappe.db.exists(
                    "Stock Ledger Entry", {"voucher_type": "Sales Invoice", "voucher_no": doc.name}
                )
                gl_exists = frappe.db.exists(
                    "GL Entry", {"voucher_type": "Sales Invoice", "voucher_no": doc.name}
                )
                # Only generate stock ledger entries if no SLE exists yet for this
                # imported submitted invoice. Existing SLE rows imply the ledger
                # was already written; rerunning it can create duplicate entries.
                try:
                    ...
                    if sle_count == 0:
                        try:
                            ...
                            doc.run_method("update_stock_ledger")
                        except Exception:
                            frappe.log_error(frappe.get_traceback(), f"update_stock_ledger (import) failed for {doc.name}")
                    elif sle_count > 0:
                        ...
                        if abs(sle_sum + sum(flt(getattr(it, "qty", 0)) for it in getattr(doc, "items", []) or [])) > 0.0001:
                            frappe.log_error(
                                f"Imported Sales Invoice {doc.name} has existing SLE mismatch; skipping update_stock_ledger to avoid duplicate entries",
                                "POS Invoice Import Ledger Guard"
                            )
                except Exception:
                    frappe.log_error(frappe.get_traceback(), f"post-import SLE quantity check failed for {doc.name}")
                # Create GL entries if missing
                if not gl_exists:
                    try:
                        doc.flags.ignore_permissions = True
                        doc.run_method("make_gl_entries")
                    except Exception:
                        frappe.log_error(frappe.get_traceback(), f"make_gl_entries (import) failed for {doc.name}")
            except Exception:
                frappe.log_error(frappe.get_traceback(), f"post-import ledger reconciliation failed for {doc.name}")
    except Exception:
        # Catch-all to ensure import flow is not blocked by our post-processing
        frappe.log_error(frappe.get_traceback(), f"post-import hooks failed for Sales Invoice {getattr(doc, 'name', '(new)')}")


def finalize_imported_sales_invoice(doc, method=None):
    """Post-insert hook for imported Sales Invoices.

    Data Import may create submitted POS invoices without the normal submit
    hooks and approval flags. This finalizer sets approval state and runs
    any missing POS/SLE/GL side-effects exactly once.
    """
    if not _is_data_import_context(doc):
        return
    ...
    try:
        ...
        if hasattr(doc, "custom_si_approval_state"):
            doc.db_set("custom_si_approval_state", "Approved", update_modified=False)
    except Exception:
        frappe.log_error(frappe.get_traceback(), f"approval state finalization failed for {doc.name}")

    try:
        create_customer_device_records(doc)
    except Exception:
        frappe.log_error(frappe.get_traceback(), f"create_customer_device_records (import finalize) failed for {doc.name}")

    try:
        update_serial_lifecycle(doc)
    except Exception:
        frappe.log_error(frappe.get_traceback(), f"update_serial_lifecycle (import finalize) failed for {doc.name}")

    try:
        update_kiosk_token_status(doc)
    except Exception:
        frappe.log_error(frappe.get_traceback(), f"update_kiosk_token_status (import finalize) failed for {doc.name}")

    try:
        increment_coupon_usage(doc)
    except Exception:
        frappe.log_error(frappe.get_traceback(), f"increment_coupon_usage (import finalize) failed for {doc.name}")

    try:
        expected_qty = sum(
            flt(getattr(item, "qty", 0)) for item in getattr(doc, "items", []) or []
        )
        sle_count = flt(frappe.db.sql(
            "SELECT COUNT(*) FROM `tabStock Ledger Entry` "
            "WHERE voucher_type=%s AND voucher_no=%s",
        ...
        if sle_count == 0:
            try:
                ...
                frappe.log_error(frappe.get_traceback(), f"update_stock_ledger (import finalize) failed for {doc.name}")
        elif abs(
            flt(frappe.db.sql(
                "SELECT IFNULL(SUM(actual_qty), 0) FROM `tabStock Ledger Entry` "
                "WHERE voucher_type=%s AND voucher_no=%s",
                ("Sales Invoice", doc.name),
            )[0][0] or 0) + expected_qty
        ) > 0.0001:
            frappe.log_error(
                f"Imported Sales Invoice {doc.name} has existing SLE mismatch; skipping update_stock_ledger to avoid duplicate entries",
                "POS Invoice Import Ledger Guard"
        ...
        frappe.log_error(frappe.get_traceback(), f"SLE quantity finalization failed for {doc.name}")
    ...
    try:
        ...
        if not gl_exists:
            try:
                ...
                frappe.log_error(frappe.get_traceback(), f"make_gl_entries (import finalize) failed for {doc.name}")
    except Exception:
        frappe.log_error(frappe.get_traceback(), f"GL finalization failed for {doc.name}")
